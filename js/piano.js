...
/**
 * X (Twitter) への共有機能
 * 既存のダウンロード処理を統合し、MIDI保存と𝕏起動を同時に行う
 */
window.shareToX = async function(id, title, mode) {
    try {
        ...
        // 3. 𝕏共有画面の展開
const saveMessage = `\n(MIDIファイルをダウンロードフォルダに保存しました。演奏音声や画面を録画して共有しましょう！)\n`;
...
sortedData.forEach((rec, index) => {
    ...
    html += `
        <tr>
            <td>${index + 1}</td>
            <td style="text-align: left; padding-left: 15px;">${rec.title || 'No Title'}</td>
            <td>${rec.mode || '-'}</td>
            <td>${rec.date || '-'}</td>
            ...
            <td><button class="table-btn-sns" onclick="window.shareToX(${rec.id}, '${safeTitle}', '${safeMode}')">𝕏</button></td>
