...
// ★ 各ノートの発音開始時刻（ms）を記録。stopNoteで最低持続時間を保証するために使う
...
export function playNote(rawNote, velocity = 0.7, isAuto = false, audioCtx, pianoGain) {
    ...
    // --- 【1 & 2. 記録・解決ロジック】 ---
    // ★★★ 修正：以前は !isAuto（ユーザー打鍵時のみ）に記録が限定されていたが、
    //   Studioの録音は「実際に聞こえた演奏全体（ガイド音含む）」を対象にするため、
    //   録音自体は isAuto に関わらず常に行うようにする。
    //   resolveGuideNote（鍵盤を弾いた時のゲーム側の判定）は、これまで通り
    //   ユーザー打鍵時のみ呼び出す。
    ...
    // ★ 診断ログ（finalVol確定後）
    if (isAuto) {
        console.log(`🎵 [診断] playNote(auto): note=${note}, velocity=${velocity?.toFixed(3)}, finalVol=${finalVol?.toFixed(3)}`);
    }
    ...
    noteStartTimes.set(note, performance.now()); // ★ 発音開始時刻を記録
...
export function stopNote(rawNote, audioCtx, isAuto = false) { 
    ...
    // ★ 引数の audioCtx は呼び出し元によって省略・誤値（true等）が渡されるケースがあるため
    //   常に window.audioContext を優先して使う
    const ctx = window.audioContext || window.pianoCtx || audioCtx;
    if (!ctx || typeof ctx.currentTime === 'undefined') return;

    // --- 【1. 記録ロジック】 ---
    // ★ playNote側と同様、ガイド音も含めて録音するため isAuto を問わず記録する
    if (typeof window.recordStudioEvent === 'function') {
    ...
    if (activeSources.has(note)) {
        ...
        // ★ 最低持続時間の保証（180ms未満で止めようとした場合は遅延させる）
        //   MIDIファイル内の極短ノート（数ms）がピアノ音源のアタック完成前に
        //   切られて「異音」になるのを防ぐ。
        //   ★★★ 修正：以前はここが50msだったため、GAME ONでブロックを一瞬タップしただけで
        //   即座に stopNote が呼ばれるケース（＝物理的な打鍵時間が音の長さを決めてしまう）で、
        //   余韻設定(180ms)より先にここで切られてしまい「ぱぷー」という短い音になっていた。
        //   余韻の下限と揃えて180msに引き上げる。
        const MIN_DURATION_MS = 180;
        const startMs = noteStartTimes.get(note) || 0;
        const elapsedMs = performance.now() - startMs;
        if (elapsedMs < MIN_DURATION_MS) {
            const delayMs = MIN_DURATION_MS - elapsedMs;
            setTimeout(() => {
                // ★ 診断ログ：stopNote の実行を遅延
                console.log(`🛑 [診断] stopNote: note=${note}, isAuto=${isAuto}, ctx_valid=${!!ctx}`);
                // 遅延後も同じノートが有効なら止める
                if (activeSources.has(note)) {
                    const { source: s2, gainNode: g2 } = activeSources.get(note);
                    const now2 = ctx.currentTime;
                    g2.gain.cancelScheduledValues(now2);
                    // ★ 修正：ここも0.04(40ms)は速すぎたため、通常の余韻(180ms/2.5)に近い値へ
                    g2.gain.setTargetAtTime(0, now2, 0.07);
                    try { s2.stop(now2 + 0.4); } catch(e) {}
                    activeSources.delete(note);
        ...
        // 手数が多い（超高速ファストジャズ）＝ 下限を0.18秒(180ms)へ（★ 0.06→0.09でもまだ
        //   不格好に短く感じられたため、さらに引き上げた）
        const noteCount = globalNoteTimestamps.length;
        let releaseTime = 0.20 - (Math.min(noteCount, 15) * 0.0013); 
        releaseTime = Math.max(0.18, releaseTime); // ★下限を0.09秒→0.18秒に引き上げ
